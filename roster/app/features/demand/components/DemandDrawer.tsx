 ...
 * "Design" §4). Called a "ドロワー" in the stage doc; rendered here as an
 * inline panel rather than an overlay dialog — roster has no dialog
 * primitive (ADR-001: no local `ui/` layer, and `@gdgjp/gdg-lib/ui` doesn't
 * export one) and a full modal isn't worth adding for one form.
 *
...
export function DemandDrawer({
  ...
  return (
    <div className="space-y-4 rounded-2xl border-2 border-gdg-blue bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm text-neutral-500">
            {rowLabel} / {trackName}
          </p>
          <h3 className="text-lg font-bold">{roleName}</h3>
        </div>
        <button
          type="button"
          onClick={onClose}
